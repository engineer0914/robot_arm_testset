...
def _main():
    ...
    target_info = [
            np.array([0, 0, 0, 90, 0, 0]),   #where from with start angle
            np.array([0, 0, 0, 0, 0, 0]),   # where to direct move distance
    ...
    robot.move_l_rel(rc, target_info[0], target_info[2], target_info[3], rb.ReferenceFrame.User0)
    if robot.wait_for_move_started(rc, 0.5).is_success():
        robot.wait_for_move_finished(rc)
    rc.error().throw_if_not_empty()
    ...
    robot.move_l_rel(rc, target_info[1], target_info[2], target_info[3], rb.ReferenceFrame.User0)
    if robot.wait_for_move_started(rc, 0.5).is_success():
        robot.wait_for_move_finished(rc)
    rc.error().throw_if_not_empty()
    ...
    target_info = [
            np.array([100, 0, 0, 30, 0, 0]),   #where from with start angle
            np.array([0, -100, 0, 0, 0, 0]),   # where to direct move distance
            200, # 시작지점까지 이동시 velocity
            200, # 시작지점까지 이동시 acceleration
            200, # 툴 직선 이동시 velocity
    ...
    robot.move_l_rel(rc, target_info[1], target_info[2], target_info[3], rb.ReferenceFrame.Tool)
    if robot.wait_for_move_started(rc, 0.5).is_success():
        robot.wait_for_move_finished(rc)
    rc.error().throw_if_not_empty()
